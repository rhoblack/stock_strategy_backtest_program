...
각 항목은 `backend/tests/acceptance/test_accuracy_policy.py` 에서 1:1 매핑 테스트로 검증합니다 (step 038에서 구축).

```text
§17.1  일중 손절 도달 정확성             → test_13_17_1_*
§17.2  일중 익절 도달 정확성             → test_13_17_2_*
§17.3  동일 봉 익절·손절 동시 도달 시 손절 우선 → test_13_17_3_*
§17.4  갭 다운 손절 시 시가 체결         → test_13_17_4_*
§17.5  거래정지 종목 매수/매도 차단      → test_13_17_5_*
§17.6  상한가 매수 차단                  → test_13_17_6_* (⚠️ xfail: BacktestConfig.allow_buy_limit_up 미구현)
§17.7  호가 단위 반올림                  → test_13_17_7_*
§17.8  거래세 시계열 적용                → test_13_17_8_*
§17.9  수정주가 사용 일관성              → test_13_17_9_*
§17.10 동시 신호 우선순위 결정론         → test_13_17_10_*
§17.11 random_seed 동일 시 동일 결과     → test_13_17_11_*
```

§17.6 상한가 매수 차단은 `BacktestConfig.allow_buy_limit_up` 옵션 미구현으로 현재 xfail 상태. backtest-engine-developer에 구현 요청 필요.
