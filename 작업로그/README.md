...
| 1 | 백엔드 핵심 엔진 (조건 5개 + StrategyEngine + 단일종목 백테스트 + Metrics) | 🔄 진행 중 (6/9) |
...
**현재 작업 중**: 없음 (Phase 1 / Step 7 — 단일 종목 BacktestEngine 골격 — 다음에 시작)

**환경 셋업 완료**: `backend/.venv/` 활성화 후 `./.venv/Scripts/python.exe -m pytest` 로 검증 가능. 184/184 통과 상태.
...
7. ⏭ **다음**: 단일 종목 BacktestEngine 골격 (날짜별 루프, exit_position 평가, ExecutionModel/Portfolio 조립)
